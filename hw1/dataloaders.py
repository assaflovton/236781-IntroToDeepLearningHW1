...
def create_train_validation_loaders(
    ...
    #  you create.
    # ====== YOUR CODE: ======
    vald_samp = torch.utils.data.SubsetRandomSampler(indices=list(range(len(dataset)))[:int(validation_ratio * len(dataset))])
    train_samp = torch.utils.data.SubsetRandomSampler(indices=list(range(len(dataset)))[int(validation_ratio * len(dataset)):])
    dl_train = torch.utils.data.DataLoader(dataset, sampler=train_samp, batch_size=batch_size,num_workers=num_workers)
    dl_valid = torch.utils.data.DataLoader(dataset, sampler=vald_samp, batch_size=batch_size,
