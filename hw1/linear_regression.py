...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        y_pred = None
        # ====== YOUR CODE: ======
        y_pred = numpy.matmul(X,self.weights_)
        # ========================
        return y_pred
    ...
    def fit(self, X, y):
        ...
        sec = self.reg_lambda * X.shape[0] * numpy.eye(X.shape[1])
        sec[0][0] = 0
        inv = numpy.linalg.inv(XtX + sec)
        Xty = numpy.matmul(Xt,y)
        w_opt = numpy.matmul(inv,Xty)
        # ========================
        self.weights_ = w_opt
        return self
...
def fit_predict_dataframe(
    ...
    # ====== YOUR CODE: ======
    col_of_target = df[target_name]
    if(feature_names is None):
        D = df.drop(columns=target_name)
        y_pred = model.fit_predict(D,col_of_target)
    else:
        y_pred = model.fit_predict(df[feature_names], col_of_target)
    # ========================
    return y_pred
...
class BiasTrickTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray):
        ...
        xb = None
        # ====== YOUR CODE: ======
        ones = np.ones((np.size(X, axis=0), 1))    # one vector
        xb = np.hstack((ones, X))       # stack the vector to the samples
        # ========================
        return xb
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    best_n_mat = corr_mat.nlargest(n + 1, target_feature)
    best_for_MEDV = best_n_mat[target_feature]
    top_n_features = best_for_MEDV.index[1:]
    top_n_corr = best_for_MEDV.values[1:]
    # ========================
...
def r2_score(y: np.ndarray, y_pred: np.ndarray):
    ...
    # ====== YOUR CODE: ======
    r2 = 1 - ((numpy.sum(numpy.power((y-y_pred),2)))/(numpy.sum(numpy.power((y-y.mean()),2))))
    # ========================
    return r2
    ...
    for lm in lambda_range:
        for deg in degree_range:
            ...
            for train_index, vald_index in kf.split(X):
                x_train_data, x_vald_data , y_train_data, y_vald_data= X[train_index], X[vald_index], y[train_index], y[vald_index]
                tansformed = sklearn.pipeline.make_pipeline(BiasTrickTransformer(),BostonFeaturesTransformer(deg),LinearRegressor(lm))
                tansformed.fit(x_train_data,y_train_data)
                y_hat = tansformed.predict(x_vald_data)
                acc = r2_score(y_vald_data,y_hat)
                loss.append(acc)
            cur_score = sum(loss) / len(loss)
            if cur_score > best_cur_score:
                best_cur_score = cur_score
                lm_best = lm
                deg_best = deg
    # ========================
    best_params = best_params = {'linearregressor__reg_lambda': lm_best,'bostonfeaturestransformer__degree': deg_best}
    return best_params
